...
%The previous achievements were obtained by using sophisticated algorithms that utilize neural networks. However, it is not always possible to use neural networks, since these can be too memory or processing power demanding. Especially, in embedded real-time systems, large neural networks can be problematic due to limited system resources. Therefore, the algorithms presented in \cite{RL:control, RL:control2, RL:atari, RL:alphago} cannot be utilized and different one must be selected. 
...
%Only few actions were used throughout this thesis in order to allow better comparison with ILC by consuming approximately the same amount of memory and for making the training faster. Figure \ref{fig:learned_pattern_torque} shows a learned compensation pattern consisting only four different actions. 



%Since the aim of the thesis is to prove that Q-learning can be used for compensation, only a small set of actions is used to make the training quick. The small action set also makes the performance comparisons between ILC and Q-learning more valid in the sense that both compensators use approximately the same amount of memory space. The number of actions is parametrized in the implemented compensator, therby changing the number of actions is easy. In the first simulations, only five actions were defined $a \in {}$

%select actions that maximize the cumulative future reward
%\begin{equation*}
%    Q^*(s_t, a_t) = \max_{\pi} E [r_t +\gamma r_{t+1} + \gamma^2 r_{t+2} + ... \; |s_t = s, a_t = a, \pi])
%\end{equation*}

\subsection{Q-learning algorithm} %  \cite{RL:control2}
...
% which in this case is the best action that the agent perceives to be able to perform.
...
%The $\epsilon$ is gradually decreased over time \cite{RL:GLIE}

%. The $\epsilon$-greedy was used to train the compensator.
%a random action is chosen with probability $1-\epsilon$. 
% The ratio between exploration and exploitation can be adjusted and this practice may lead to faster converge.
% can be adjusted by declaring a parameter $k$

%Therefore, an exploration policy is needed for deciding whether the agent should try something new, or if it should utilize already learnt information. An $\epsilon$-geedy exploration policy was used. The $\epsilon$-geedy is policy in which a random action is chosen with probability $\epsilon$, and $\epsilon$ is gradually decreased over time \cite{RL:GLIE}.

%In practice, the Q-learning is used to overcome two problems. Firstly, it is used to train the agent to automatically find states when compensation should be applied. Since the agent determines the states itself, the measured pulsations can have lag from real pulsations. Secondly, the agent also learns to select actions corresponding to magnitudes, that minimizes the torque pulsation at certain state. Learning to do this may require data from numerous iterations, yet this should not be a problem in case of an electric motor as it produces great amounts of data just by rotating continuously.

%In practice, the Q-learning is used to train the agent to automatically select actions corresponding to magnitudes, that minimizes the torque pulsation at certain state. Learning to do this may require data from numerous iterations. However, the data sparseness is not a problem, since electric motor as it produces great amounts of data just by rotating continuously.

%yet this should not be a problem in case of an electric motor as it produces great amounts of data just by rotating continuously.

\subsection{Torque based learning scheme}


%An example of use of the generated actions can be seen in Fig. \ref{fig:learned_pattern_torque}. 
%Only five actions were generated between $[-0.23, 0.23]$. The agent used four of these actions for compensation. The dots represent instants where the agent was able to select an action. In total, there are 100 dots, since the rotor angle was quantized to 100 states. The selected actions clearly form a waveform pattern, which indicates that the agent is not selecting actions by random.
...
%The compensator is able to significantly reduce harmonics, even with rough approximations. The policy producing compensation pattern in Fig. \ref{fig:learned_pattern_torque}, was used to compensate sixth harmonic in simulated environment. The compensator was able to reduce the harmonic approximately to one fourth from the original, as can be seen in the Fig. \ref{Fig:Qlr-torque-compensation}. It appears that large steps are not a barrier for compensation. Therefore, the number of actions can be set to be low, if faster training time and smaller memory consumption are desired with the expense of compensation performance.
%\begin{figure}
%\centering
%\begin{subfigure}{0.5\textwidth}
%  \centering
%  \includegraphics[width=\textwidth]{images/Qlr-no-compensation-f.pdf}
%  \caption{uncompensated, FFT}
%  \label{fig:sub1}
%\end{subfigure}%
%\begin{subfigure}{.5\textwidth}
%  \centering
%  \includegraphics[width=\textwidth]{images/Qlr-t-compensation-f.pdf}
%  \caption{compensated, FFT}
%  \label{fig:sub2}
%\end{subfigure}
%\caption{Torque trained compensator disabled and enabled}
%\label{Fig:Qlr-torque-compensation}
%\end{figure}
...
%In reinforcement learning, the RL-algorithm needs to find an optimal policy by interacting with the MDP \cite{RL:Single-step}.

%Q-learning is an off-policy algorithm, which conceptually allows exploration by using hypothetical actions \cite{RL:Single-step}. This property is beneficial in applications where exploration may be dangerous to do otherwise, as it still makes it possible to train the agent.

%In Q-learning the object is to estimate Q-values, for an optimal policy \cite{RL:Watkins-Q}.
...
%However, optimal learning rate can be only achieved by selecting the parameters individually for all cases.

%  Two of these motors roughly modelled the motors used in experiments. It was found that the same hyperparameters could be also used with the real motors, although faster learning was achieved by adjusting the parameters.
...
%The learning appears to be fastest and most stable with values $k=200$ and $k=500$, hence $k=300$ was used in experiments.
\begin{figure}[htb] 
    \centering
...
%It is good to note that hyperparameters can be selected simply by testing what works the best in the actual motor drive system. This approach can yield better parameter values than simulations based analysis, since it is difficult to accurately model the actual system requiring compensation.

%\subsection{Automating the choke-gain search}
%Motor nominal torque as an input. Calculate e.g. 20 percent value. Use this value as an upper limit. Sweep from zero to upper limit. Record each op. reward average. If average is going worse twice, or exceeding the limit, then stop. Pick the gain producing the best average.


%\subsection{Training}

%It was found that compensation performance is worse, if high rotation speed is used during training phase. The performance penalty can be potentially explained with state skips that occur in high speeds. Due to fast rotation speed, not all states gets visited and thus reduced amount of Q-values gets updated. With lower rotation speed, no such skipping occurs. The crucial point between high and low speeds depends on number of states. For example, with $100$ states the rotor should always rotate less than $0.01 \cdot 2\pi \approx 0.06$ rad during iteration, in order not to skip states.


%Can be trained off-line or on-line. It is also possible pretrain by using simulator and the finalize training on-line. Cogging is static so it can be learnt in simulator, whereas dynamic effects such as temperature related changes cannot be learnt.



% Episode length:
%First tell what episode is ...
%Negligible amount of time is spent in startup procedure when considering normal lifetime usage of a machine. Therefore, it is not very beneficial to try optimize startup behaviour. Moreover, trying to learn during startup can surprisingly degrade the overall performance of the compensator. This is because wheights (*fix: weights do not try to generalize*) generalize the problem, so learning completely different behaviour from normal, offsets the weights from the optimal values for normal use. Therefore, the startup procedure should not be tried to learnt.


% In [joku] it was also observed that Q-learning performs badly, if the environment is POMDP. Thus, it can be concluded that Q-learning should not be used as is for learning the compensation pattern from the speed signal. Some modifications has to be made. At this point, it is feasible to question if some different RL-algorithm should be used. However, the Q-learning was specifically chosen due to its simple structure, which allows the solution to be implemented on already commissioned drives that can be years old. Therefore, resorting to more sophisticated algorithms, which often use neural networks, was not an option. Fortunately, the problem can be circumvented and it is not necessary to move on to more complex schemes.
\clearpage
